...
Consider the material model illustrated in \cref{fig:SimpleBondpair} in which every bond vector emanating from a point is connected by a rotational spring to its opposite emanating from that same point.  These points and bonds are illustrated in the context of a plate in \cref{fig:BondPairPlate}.
%
\begin{figure}[tbp]
    \centering
...
\end{figure}
%
If we call the deformed angle between these bonds \(\theta\), and choose the potential energy of that spring to be \todo{Need to define $\boldsymbol{\xi}$ somewhere} \(w(\boldsymbol{\xi}) = \omega(\boldsymbol{\xi})\alpha [1 + \cos(\theta) ] \) for the bond pair $\boldsymbol{\xi}$ and $-\boldsymbol{\xi}$, we can recover the non-ordinary force state proposed by Silling in \cite{silling2007peridynamic} and analyzed in \cite{jogrady2014a} by taking the Fr\'echet derivative. For the derivation and a description of the Fr\'echet derivative see \ref{sec:frechet}.
%
\begin{align}
...
\end{equation}
%
By matching the energy of a plate in shear deformation, we can relate \(\beta\) to the shear modulus and thickness of the plate.  Following the example of \cite{silling2007peridynamic}, we begin with a plate \todo{should this be shell?} under pure in-plane shear.  In Einstein notation, the strain energy of this plate is
%
%\begin{align*}
...
Combining the bending and extension models allows for the description of more complex behaviors, particularly the stiffening effect of in-plane tension on the transverse bending of a plate.  Consider a single bond-pair in the combined model shown in Fig.~\ref{fig:hybridmodel}.
%
\begin{figure}[tbp]
  \centering
...
\end{align}
%
\todo{I say, to avoid the question of ``How to you select $\psi$'', let's leave it equal to $\omega$ at this point.  Then $m$ is also identical.} The weighting function \(\psi(\xi)\) performs a role similar to that of \(\omega(\xi)\) from the previous section, but need not be identical.
...
\end{equation}
%
We can take the Fr\'{e}chet derivative \todo{Can you show the details in the Appendix?  Also, should the $dA$ be there?} to produce a correction force vector state
%
\begin{equation}
